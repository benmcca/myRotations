...
class MusicDataService {
    ...
    find(query, by = "trackName", page = 0) {
        return axios.get(
          `${process.env.REACT_APP_BACKEND_URL}/api/v1/bsm25/movies?${by}=${query}&page=${page}`
        );
    }
    
    createComment(data) {
        return axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/v1/bsm25/music/comments`, data)
    }

    updateComment(data) {
    return axios.put(`${process.env.REACT_APP_BACKEND_URL}/api/v1/bsm25/music/comments`, data)
    }
